...
{
    public partial class ConnectorPayout : System.Web.UI.Page
    {
        ...
        private void BindGrid(int id)
        {
            ...
            var result = oDataAccess.QuerySPDynamic<Payouts_Add>("sp_getpayoutsforbranchbyconid", parameters);
            if (result != null)
            {
                ...
                for (int i = 0; i < payouts_s.Count; i++)
                {
                    ...
                    //parameters = new DynamicParameters();
                    //parameters.Add("Id", payouts_s[i].UserId);
                    //var connectorDetail = oDataAccess.QuerySingleOrDefaultSPDynamic<Connector_Add>("sp_getconnectorby_id", parameters);
                    //if (connectorDetail != null)
                    //{
                    //    payouts_s[i].ConnectorPanNo = connectorDetail.PanNo;
                    //}
                    //parameters = new DynamicParameters();
                    //parameters.Add("Id", CaseDetail.EmpId);
                    //var empDetail = oDataAccess.QuerySingleOrDefaultSPDynamic<Employee_Add>("sp_getemployeeby_id", parameters);
                    //if (empDetail != null)
                    //{
                    //    payouts_s[i].EmpName = empDetail.Name;
                    //}
                }
