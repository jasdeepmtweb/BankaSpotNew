...
{
    public partial class MapEmpToConnectorCase : System.Web.UI.Page
    {
        ...
        private void BindGrid(int Id)
        {
            ...
            var result = oDataAccess.QuerySPDynamic<Employee_Add>("sp_getallemployee", parameters);
            result = result.Where(x => x.EmployeeStatus.Equals("1"));
            ...
            if (Page.IsValid)
            {
                try
                {
                    ...
                    if (Request.QueryString["id"] != null)
                    {
                        ...
                        parameters.Add("p_ConnectorId", Request.QueryString["cid"]);
                        parameters.Add("p_EmployeeId", ddlEmployee.SelectedValue);
                        parameters.Add("p_CreatedOn", indianTime);

                        oDataAccess.ExecuteSPDynamic("sp_insert_connectorcasehistory", parameters);
                        ...
                        ClientScript.RegisterStartupScript(this.GetType(), "myalert", "alert('Added');window.location.href='CasesFromConnector.aspx';", true);
                    }
                }
                catch (Exception ex)
                {
