...
{
    public partial class MakeEmployeeWithdrawal : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                Response.Redirect("ShowEmployees.aspx");
            }
            if (!IsPostBack)
            {
        ...
        private void GetPayoutsAndWithdrawal(string Id)
        {
            ...
            var withdrawal = oDataAccess.QuerySingleOrDefaultSPDynamic<dynamic>("sp_gettotalwithdrawal_foremp", parameters);
            if (withdrawal != null)
            {
                ...
                try
                {
                    if (Request.QueryString["id"] != null)
                    {
                        if (Convert.ToDouble(txtWithdrawal.Text) < Convert.ToDouble(txtAvailableBalance.Text))
                        {
                            ...
                            DataAccess oDataAccess = new DataAccess();
                            oDataAccess.ExecuteSPDynamic("sp_insert_empwithdrawal", parameters);

                            ClientScript.RegisterStartupScript(this.GetType(), "myalert", "alert('Done');window.location.href='ShowEmployees.aspx';", true);
                        }
                        else
                        {
