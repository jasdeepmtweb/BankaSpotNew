...
{
    public partial class ShowFreelancerPayout : System.Web.UI.Page
    {
        ...
        private void GetData()
        {
            ...
            var result = oDataAccess.QuerySPDynamic<Payouts_Add>("sp_getfreelancerpayoutsfor_admin");
            if (result != null)
            {
                ...
                for (int i = 0; i < payouts_s.Count; i++)
                {
                    ...
                    //{
                        payouts_s[i].OriginalPayout = payouts_s[i].AmountDisbursed * (payouts_s[i].FreelancerCom / 100);
                        payouts_s[i].TDS = (payouts_s[i].OriginalPayout * 0.05);
                    //}

                    //parameters = new DynamicParameters();
                    //parameters.Add("p_Id", payouts_s[i].UserId);
                    //var connectorDetail = oDataAccess.QuerySingleOrDefaultSPDynamic<Freelancer_Add>("sp_getfreelancerby_id", parameters);
        ...
        protected void btedt_Click(object sender, EventArgs e)
        {
            ...
            Session["lastPage"] = "ShowFreelancerPayout.aspx";
